...
**已知问题**：
- 4 个事件触发 Patch 使用 Memory/Storyteller 模块的翻译 key，未安装这些模块时显示原始 key
- Patch_PersonalitySkill 查找 Pawn 方式低效（O(maps × colonists)），且访问私有字段
- Patch_PersonalityInjury 无严重度过滤，轻微伤也触发评估
- Patch_PersonalityIncident 无事件类别过滤，非威胁事件也触发评估
...
│   ├── Patch_PersonalityInjury.cs   Postfix on HediffSet.AddHediff → 受伤/患病触发
│   ├── Patch_PersonalitySkill.cs    Prefix+Postfix on SkillRecord.Learn → 技能升级触发
│   ├── Patch_PersonalityIncident.cs Postfix on IncidentWorker.TryExecuteWorker → 事件触发
│   └── Patch_PersonalityDeath.cs    Postfix on Pawn.Kill → 亲近者死亡触发
└── Debug/
...
| 枚举值 | 对应设置开关 | 对应 Patch | 说明 |
|--------|------------|-----------|------|
| `Injury` | `enableInjuryTrigger` | Patch_PersonalityInjury | 受伤/患病触发 |
| `Skill` | `enableSkillTrigger` | Patch_PersonalitySkill | 技能升级触发 |
| `Incident` | `enableIncidentTrigger` | Patch_PersonalityIncident | 重大事件触发（默认值） |
| `Death` | `enableDeathTrigger` | Patch_PersonalityDeath | 亲近者死亡触发 |

**ContextRequest 参数**：`Scenario=Personality, ExcludeKeys=["personality_state"], MaxTokens=600, Temperature=0.8f`

### 事件触发 Patch 详情

#### Patch_PersonalityInjury

```csharp
[HarmonyPatch(typeof(HediffSet), "AddHediff")]
static void Postfix(HediffSet __instance, Hediff hediff)
// 1. 检查 enableInjuryTrigger
// 2. 检查 hediff != null && pawn.IsColonist
// 3. 调用 comp.TriggerEvent("RimMind.Memory.Trigger.Contracted".Translate(...), TriggerEventType.Injury)
// ⚠️ 无严重度过滤：1点擦伤也会触发
// ⚠️ 使用 Memory 模块的翻译 key
```

#### Patch_PersonalitySkill

```csharp
[HarmonyPatch(typeof(SkillRecord), "Learn")]
// Prefix: 记录 PreLevels[hash] = levelInt
// Postfix: 检查 levelInt > preLevel，遍历所有地图所有殖民者查找 SkillRecord 所属 Pawn
// ⚠️ O(maps × colonists × skills) 查找，性能差
// ⚠️ 访问 Pawn_SkillTracker.skills 私有字段（AccessTools.Field）
// ⚠️ 使用 Memory 模块的翻译 key
```

#### Patch_PersonalityIncident
...
// 1. 检查 __result == true（事件成功执行）
// 2. 检查 enableIncidentTrigger
// 3. 检查 parms.target is Map
// 4. 对地图上所有 FreeColonists 调用 TriggerEvent
// ⚠️ 无事件类别过滤：短路/访客等非威胁事件也触发
// ⚠️ 使用 Storyteller 模块的翻译 key
```

#### Patch_PersonalityDeath

```csharp
[HarmonyPatch(typeof(Pawn), "Kill")]
static void Postfix(Pawn __instance, DamageInfo? dinfo, Hediff exactCulprit)
// 1. 检查 enableDeathTrigger
// 2. 检查 __instance.Map != null
// 3. 遍历地图上所有 FreeColonists，检查 DirectRelations 是否包含死者
// 4. 对有关系的殖民者调用 TriggerEvent
// ⚠️ 使用 Memory 模块的翻译 key
// ⚠️ 只检查 DirectRelations，不检查非关系的目击者
```
...
| personality_task | TaskInstruction 静态上下文 | L0_Static 层，优先级 0.95，仅 Personality 场景注入 |
...
3. `RegisterContextProviders()` — 注册 3 个 PawnContextProvider + 1 个静态上下文
...
```
事件源:
  HediffSet.AddHediff ──→ Patch_PersonalityInjury ──→ TriggerEvent(context, Injury)
  SkillRecord.Learn ───→ Patch_PersonalitySkill ───→ TriggerEvent(context, Skill)
  IncidentWorker ──────→ Patch_PersonalityIncident → TriggerEvent(context, Incident)
  Pawn.Kill ───────────→ Patch_PersonalityDeath ───→ TriggerEvent(context, Death)
  每日定时 ────────────→ CompTick 内部检测
...
| enableInjuryTrigger | bool | true | 受伤触发（⚠️ 无严重度过滤） |
| enableSkillTrigger | bool | true | 技能升级触发（⚠️ 查找低效） |
| enableIncidentTrigger | bool | true | 事件触发（⚠️ 无类别过滤） |
...
- `RimMind.Personality.Patches` — Harmony 补丁（Comp 注入 + 4 个事件触发 Patch）
...
- `Patch_PersonalityInjury`：Postfix on `HediffSet.AddHediff`
- `Patch_PersonalitySkill`：Prefix+Postfix on `SkillRecord.Learn`
- `Patch_PersonalityIncident`：Postfix on `IncidentWorker.TryExecuteWorker`
- `Patch_PersonalityDeath`：Postfix on `Pawn.Kill`
...
- ⚠️ 4 个事件 Patch 使用 Memory/Storyteller 模块的翻译 key，不在本模块语言文件中
...
15. **跨模块翻译 key**：4 个事件 Patch 使用 Memory/Storyteller 的翻译 key，未安装时显示原始 key
16. **rimTalkSynced 死字段**：序列化但从未读写，可考虑移除
17. **Profile 不清理**：死亡/离队 Pawn 的 Profile 永不清理，长期游戏存档膨胀
18. **Injury Patch 无过滤**：轻微伤也触发评估，频繁战斗时可能过度调用 API
19. **Incident Patch 无过滤**：非威胁事件也触发评估
20. **Skill Patch 低效**：O(maps × colonists) 查找 + 访问私有字段
