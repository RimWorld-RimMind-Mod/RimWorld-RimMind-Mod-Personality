...
├── Comps/
│   ├── CompAIPersonality.cs         ThingComp：评估触发 + 事件监听 + 每日抖动
...
// 核心方法
override void CompTick()     // 检测：每日定时（含抖动）或 事件触发
...
5. 每日定时触发（`enableDailyEval`，含确定性抖动避免同时触发）
6. 或事件触发（受伤/技能/事件/死亡），带 1200 tick 冷却
